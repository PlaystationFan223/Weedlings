...
#include "../../source/SexyAppFramework/Common.cpp" // get MkDir and some other helper functions.
#include "../../source/PakLib/PakInterface.cpp"
#include "../../source/SexyAppFramework/MTRand.cpp"
#include "../../source/SexyAppFramework/Debug.cpp"
#include "../../source/SexyAppFramework/CritSect.cpp"
#include "PopPak.h"
using namespace std;
...
int enc_fwrite(const void* theData, size_t theElemSize, size_t theCount, FILE* theFP)
{
	...
	for (int i = 0; i < aSize; i++)
		aBuf[i] = ((char*) theData)[i] ^ gEncryptPassword; // post increment
...
{
	std::string aPackName = std::string(aPackNameC, aPackName_length);
	std::string aDirName = std::string(aDirNameC, aDirName_length);
	retFlag = true;
...
extern "C" __declspec(dllexport) int Unpackage(const char* aPackNameC,size_t aPackName_length, const char* aDirNameC, size_t aDirName_length, bool& retFlag)
{
	...
	for (PakRecordMap::iterator anItr = anInterface.mPakRecordMap.begin(); anItr != anInterface.mPakRecordMap.end(); ++anItr)
	{
		...
		if (anItr->first == Sexy::StringToUpper(GetFileName(aPackName)))
			continue;
...
int main(int argc, char* argv[])
{
	...
	while (argc >= anArgPos + 1)
	{
		...
		{			
			...
			{
				anArgPos++;
				gEncryptPassword =std::stoi( argv[anArgPos]);
			}
			else
			{
	...
	cout << StrFormat("Password: %x", gEncryptPassword).c_str() << endl;
	if (gEncryptPassword == 0xF7)
		cerr << StrFormat("WARNING: %s is the Default Password and is NOT allowed for Distribution!", gEncryptPassword).c_str() << endl;
	...
	{
		bool retFlag;
		int retVal = Package(aPackName.c_str(), aPackName.length(), aDirName.c_str(), aDirName.length(), retFlag);
	...
	{
		bool retFlag;
		int retVal = Unpackage(aPackName.c_str(), aPackName.length(), aDirName.c_str(), aDirName.length(), retFlag);
		if (retFlag) return retVal;
	}
