...
{
	std::fstream aXMLStream(theFileName.c_str(), std::ios::out);
	...
	aXMLStream << "<Defines>" << std::endl;

	DefinesMap::iterator anItr = mDefinesMap.begin();
	while (anItr != mDefinesMap.end())
	{
		aXMLStream << "\t<Define>" << std::endl;
		aXMLStream << "\t\t<Name>" << XMLEncodeString(anItr->first).c_str() << "</Name>" << std::endl;
		aXMLStream << "\t\t<Value>" << XMLEncodeString(anItr->second).c_str() << "</Value>" << std::endl;
		aXMLStream << "\t</Define>" << std::endl;
		++anItr;
	}
